...
NEW in this version:
• The exogenous resistance load is **angle-dependent**, not time-based.
  It applies, in piecewise fashion (with wrap-around), the term:
      K R1 * sin(π*(θ - center)/θ_max) if |θ - center| ≤ θ_max
  for centers at 0, 2π/3, 4π/3 (default), and 0 otherwise.
  Amplitude K*R1 is configured as POS_PROF_AMP_FRAC * POMDP_TAU_SAT.
"""
...
# --- NEW: positional resistance profile (instead of time sine) ----------------
# Amplitude KR1 = POS_PROF_AMP_FRAC * POMDP_TAU_SAT
POS_PROF_AMP_FRAC       = 0.6
...
class HingeLoad:
    """
    Angle-dependent resistance profile (piecewise, with wrap-around):
        τ_pos(θ) = KR1 * sin(π*(θ - θc)/θ_max)  if |wrap(θ - θc)| ≤ θ_max,
                    0                             otherwise,
    evaluated for θc ∈ {0, 2π/3, 4π/3}.
    KR1 is set via POS_PROF_AMP_FRAC * tau_sat.
    """
    def __init__(self, model, data, joint_name="hinge_joint",
    ...
    def enable(self, t_now: float):
        ...
        print(f"[load] Positional resistance enabled (KR1={self.amp:.2f} N·m, θ_max={deg:.1f}°, centers={cdeg}).")

    @staticmethod
    def _nearest_wrapped(theta: float, center: float) -> float:
        """Return center shifted by 2πk to be nearest to theta."""
    ...
    def step(self, t_now: float):
        ...
        for c in self.centers:
            ...
            if abs(d) <= self.theta_max:
                tau = self.amp * math.sin(math.pi * (d / self.theta_max))
                break  # piecewise: first matching window
