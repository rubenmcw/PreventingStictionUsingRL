...
• Angle-dependent resistance with selectable shape:
  ...
  using the nearest 2π-wrapped copy so the windows repeat over rotation.
"""
...
# POMDP model / weights (also re-used by MPC weights for minimal diffs)
...
POS_PROF_SHAPE          = "sin"   # "saw" (default) or "sin"
...
class HingeMPC:
    ...
    • Small integral bias (like the POMDP’s) to kill steady-state error.
    Interface mirrors HingePOMDP/PID so the rest of the code remains untouched.
    """
    def __init__(self, model, data, joint_name="hinge_joint",
        ...
        # Belief vector retained for CSV schema (zeros)
        self.belief = [0.0]*N_REG
        # Friction params attribute kept so plotting path remains identical (though unused)
    ...
    def _rollout_cost_const_u(self, th0: float, w0: float, u: float, sp_hold: float, u_prev: float) -> float:
        ...
        th = th0; w = w0
        csum = self.r_du * (u - u_prev)**2  # Δu penalty once, like previous design
    ...
    def step(self):
        ...
        # Small integral bias to remove steady-state error (like prior versions)
        e_now = (self.sp - q)
...
class HingePOMDP:
    ...
    def step(self):
        # unused in this script (kept for interface compatibility)
        pass
...
class GraspSM:
    ...
    def __init__(self, model, data, ur: UR10, hand: BarrettHand,
                 ...
                 hinge_pomdp, hinge_load: HingeLoad,
    ...
    def _maybe_post_plot(self, t: float):
        ...
        # expected friction torque (belief-weighted) — zero under MPC/PID since belief==0
        tau_f_exp = 0.0
        if hinge_en:
            if hasattr(self.hinge, "params"):
                for j in range(N_REG):
...
def simulation_thread(model, data, total_time_s, realtime, plotter_ui: LivePlotterUI):
    ...
    # ********** THE ONLY FUNCTIONAL CHANGE HERE: use MPC instead of PID/POMDP **********
