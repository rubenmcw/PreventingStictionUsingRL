...
# -*- coding: utf-8 -*-
"""
UR10e + Barrett hand + panel hinge (PID + CSV logging + SINUSOIDAL (time) RESISTANCE):
--------------------------------------------------------------------------------------
This version is **identical** to the prior MPC version except the hinge controller
is swapped to the previously used PID with basic anti‑windup.

Preserved features:
(1) likelihood in torque units with filtered acceleration (telemetry compatibility)
(2) runtime hinge inertia estimate J_eq
(3) risk = actuator saturation on |u|
(5) Δu penalty infrastructure retained (unused by PID)
(6) dense action grid near zero (unused by PID but kept for consistency)
(7) small bounded integral bias → realized as PID I‑term with anti‑windup
• L1 error cost term and excessive‑error risk channel (risk telemetry)
• Comprehensive CSV logging (~10 Hz) named after this .py ('.py'→'.csv')
• TIME‑BASED sinusoidal resistance load at the hinge, unchanged.
"""
...
# --- hinge / friction params (shared across designs) -------------------------
...
# friction magnitudes scaled vs. torque cap (kept for telemetry)
...
POMDP_W_RISK       = 1.5
POMDP_W_DU         = 2e-4
POMDP_U_SMOOTH_ALPHA = 0.0
POMDP_H_CTRL       = 12
POMDP_GAMMA        = 0.98

# --- PID gains (previously used style, with anti‑windup) ---------------------
PID_KP = 10.0
PID_KI = 2.0
PID_KD = 1.0
PID_I_LIMIT_FRAC = 0.35  # fraction of τ_sat for integral clamp
...
class HingePID:
    """Hinge PID with basic anti‑windup (conditional integration).
    Compatible interface with prior controllers for minimal diffs.
    """
    def __init__(self, model, data, joint_name="hinge_joint",
        ...
        self.I = 0.0
        self.I_limit = float(i_limit_frac) * self.tau_sat
        ...
        self.bias_u = 0.0         # report I-term as "bias_u"
        self.belief = [0.0]*5     # REG count
    ...
    def enable_and_capture(self):
        ...
        self._sp_prev = self.sp
        self.I = 0.0
        self.last_tau_cmd = 0.0
        self._prev_v = dq
        self._a_hat = 0.0
        self.enabled = True
        print(f"[hinge-PID] Enabled at q={q:.4f} rad; τ_sat={self.tau_sat:.1f} N·m; "
              f"Kp={self.kp:.2f}, Ki={self.ki:.2f}, Kd={self.kd:.2f}")

    def set_target_relative(self, dq: float):
        lo, hi = (-np.inf, np.inf)
        if self.model.jnt_limited[self.jid] == 1:
            lo, hi = self.model.jnt_range[self.jid]
        q = float(self.data.qpos[self.qadr])
        self.sp_goal = clamp(q + float(dq), lo, hi)
        print(f"[hinge-PID] Target REL → goal={self.sp_goal:.4f} rad")
    ...
    def set_target_absolute(self, q_abs: float):
        ...
        print(f"[hinge-PID] Target ABS → goal={self.sp_goal:.4f} rad")
    ...
    def step(self):
        ...
        # PID terms
        e = self.sp - q
        sp_dot = (self.sp - self._sp_prev) / dt
        self._sp_prev = self.sp
        e_dot = sp_dot - dq

        # Conditional integration (anti‑windup):
        # compute u without clamping to evaluate saturation tendency
        u_preI = self.kp*e + self.kd*e_dot
        u_unsat = u_preI + self.ki*self.I

        # only integrate if not saturating in the same direction
        will_saturate = abs(u_unsat) > self.tau_sat + 1e-12
        if not will_saturate or (u_unsat*e) < 0.0:
            self.I += e * dt
            self.I = clamp(self.I, -self.I_limit/self.ki if self.ki > 1e-12 else 0.0,
                                 self.I_limit/self.ki  if self.ki > 1e-12 else 0.0)
        ...
        self.last_tau_cmd = float(u)
        self.bias_u = float(self.ki * self.I)

        # Accel estimate for telemetry
        a_raw = (dq - self._prev_v) / dt
        self._prev_v = dq
        self._a_hat = 0.2*a_raw + 0.8*self._a_hat

        # risks (telemetry only)
        self.last_risk_act = float(1.0 if abs(u) > TAU_MAX_FOR_RISK else 0.0)
        self.last_risk_err = float(1.0 if abs(e) > ERROR_RISK_THRESH_RAD else 0.0)


# =============================================================================
# ------------------------------ POMDP LOGIC (unused by PID; kept) ------------
# =============================================================================
...
class GraspSM:
    ...
    3) Settle   → release hinge freeze; enable PID; start sinusoidal load
    4) Drive
    """
    def __init__(self, model, data, ur: UR10, hand: BarrettHand,
                 ...
                 hinge_ctrl: HingePID, hinge_load: HingeLoad,
    ...
    def _maybe_post_plot(self, t: float):
        ...
        # expected friction torque (belief-weighted) — zeros under PID
        tau_f_exp = 0.0
        if hinge_en and hasattr(self.hinge, "params"):
            for j in range(N_REG):
                b_j = self.hinge.belief[j] if j < len(self.hinge.belief) else 0.0
                tau_f_exp += b_j * friction_torque(angle, dq, j, self.hinge.params)
...
def simulation_thread(model, data, total_time_s, realtime, plotter_ui: LivePlotterUI):
    ...
    # *** ONLY DIFFERENCE: use PID controller instead of MPC/POMDP ***
    hinge_ctrl = HingePID(model, data, joint_name=FREEZE_PANEL_JOINT,
                          dt=CTRL_DT,
                          tau_sat=POMDP_TAU_SAT,
                          sp_speed=POMDP_SP_SPEED_RAD_S,
                          kp=PID_KP, ki=PID_KI, kd=PID_KD,
    ...
    # Use the TIME‑BASED sinusoidal load (unchanged)
