...
class HingePID:
    """PID controller with setpoint ramp, derivative filtering, and anti‑windup.
    Interface mirrors HingePOMDP so the rest of the code (SM, logging, plots)
    remains unchanged.
    """
    def __init__(self, model, data, joint_name="hinge_joint",
                 ...
                 kp=30.0, ki=8.0, kd=1.2,
                 d_tau=0.02,          # derivative LPF time constant (s)
                 aw_k=6.0,            # anti‑windup back‑calc gain (1/s)
                 i_lim_frac=0.6,      # integrator clamp fraction of tau_sat
                 J_est: float = 0.0,  # kept for telemetry consistency
                 D_est: float = 0.0):
        self.model = model; self.data = data
        ...
        self.aw_k  = float(aw_k)
        self.i_lim = float(i_lim_frac) * self.tau_sat
        ...
        # telemetry compatibility (match POMDP fields)
        self.bias_u = 0.0
        self.J_est  = float(J_est)
        self.D_est  = float(D_est)
        self.belief = [0.0]*N_REG  # keep CSV/plots schema identical

        # risks
        self.last_risk_act = 0.0
        self.last_risk_err = 0.0

        # precompute derivative LPF alpha
        if self.d_tau <= 0.0: self.d_alpha = 0.0
        else:                 self.d_alpha = math.exp(-self.dt / self.d_tau)
    ...
    def enable_and_capture(self):
        ...
        self.last_tau_cmd = 0.0
        self.enabled = True
        print(f"[hinge-PID] Enabled at q={q:.4f} rad; tau_sat={self.tau_sat:.1f} N·m; "
    ...
    def step(self):
        ...
        sp_dot = (self.sp - self.prev_sp) / dt
        self.prev_sp = self.sp

        # Derivative on measurement (filtered)
        self.d_filt = self.d_alpha * self.d_filt + (1.0 - self.d_alpha) * dq
        v_err = sp_dot - self.d_filt

        # PID
        e = (self.sp - q)
        u_unsat = self.kp*e + self.i_state + self.kd*v_err

        # Saturation
        u = clamp(u_unsat, -self.tau_sat, self.tau_sat)

        # Anti‑windup (back‑calculation) + clamp
        self.i_state += (self.ki*e + self.aw_k*(u - u_unsat)) * dt
        self.i_state = clamp(self.i_state, -self.i_lim, self.i_lim)

        # Apply torque
        self.data.qfrc_applied[self.vadr] += u
        self.last_tau_cmd = u
        self.bias_u = self.i_state  # keep same telemetry name

        # Accel estimate for tau_dyn telemetry (not used in control)
        a_raw = (dq - self.prev_v) / dt
        self.prev_v = dq
        self._a_hat = 0.2*a_raw + 0.8*self._a_hat

        # Risks consistent with POMDP logging
...
class GraspSM:
    ...
    def _maybe_post_plot(self, t: float):
        ...
        if hinge_en:
            for j in range(N_REG):
                tau_f_exp += self.hinge.belief[j] * friction_torque(angle, dq, j, self.hinge.params) if hasattr(self.hinge, "params") else 0.0
...
def simulation_thread(model, data, total_time_s, realtime, plotter_ui: LivePlotterUI):
    ...
    # ********** THE ONLY FUNCTIONAL CHANGE: use PID instead of POMDP **********
    hinge_ctrl = HingePID(model, data, joint_name=FREEZE_PANEL_JOINT,
                          dt=CTRL_DT, tau_sat=POMDP_TAU_SAT,
                          sp_speed=POMDP_SP_SPEED_RAD_S,
                          kp=30.0, ki=8.0, kd=1.2,
                          d_tau=0.02, aw_k=6.0, i_lim_frac=0.6,
                          J_est=J_eq, D_est=0.0)
    # *************************************************************************
