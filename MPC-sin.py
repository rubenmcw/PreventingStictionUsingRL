...
# -*- coding: utf-8 -*-
"""
UR10e + Barrett hand + panel hinge (MPC + CSV logging + SINUSOIDAL (time) RESISTANCE):
--------------------------------------------------------------------------------------
This version keeps your earlier improvements and CSV logging, but:
• Uses an MPC hinge controller (short-horizon, constant‑input enumeration).
• Uses the resistance profile from your provided script: a TIME‑BASED sinusoidal
  load at the hinge (amp=f·τ_sat, freq=LOAD_SINE_FREQ_HZ, phase=LOAD_PHASE).

Other details preserved:
(1) likelihood in torque units with filtered acceleration (kept in code paths)
(2) runtime hinge inertia estimate J_eq
(3) risk = actuator saturation on |u|
(5) Δu penalty instead of output low-pass
(6) dense action grid near zero
(7) small bounded integral bias
• L1 error cost term in the horizon (small)
• Excessive-error risk channel
• Comprehensive CSV logging (~10 Hz) named after this .py ('.py'→'.csv')
"""
...
# friction magnitudes scaled down vs. torque cap (kept for telemetry compat)
POMDP_FRIC = StribeckParameters(Fc=0.35 * POMDP_TAU_SAT,
                                Fs=0.60 * POMDP_TAU_SAT)

# Likelihood / belief (kept for compatibility in telemetry)
SIGMA_RESIDUAL      = 0.30         # N·m residual noise scale
BELIEF_FLOOR        = 1e-6
P_MAX_RISK          = 0.4
TAU_MAX_FOR_RISK    = 1.3 * POMDP_TAU_SAT

# --- excessive‑error risk + soft L1 error cost
...
# Weights / model (also re-used by MPC)
...
POMDP_H_CTRL       = 12     # horizon
POMDP_GAMMA        = 0.98   # discount

# --- SINUSOIDAL (time) load profile (from your script) -----------------------
LOAD_SINE_FREQ_HZ   = 0.5
LOAD_SINE_AMP_FRAC  = 0.35
LOAD_PHASE          = 0.0
...
class HingeMPC:
    """Receding-horizon MPC with constant control over a short horizon.
    • Linear model: [θ, ω]ᵀ with ω̇ = (u - D_est ω)/J_est
    • Cost (per step, discounted): Qθ·(θ−sp)² + Qω·ω² + Ru·u² + RΔu·(u−u_prev)² + L1·|θ−sp|
    • Constraints: u ∈ [−τ_sat, +τ_sat]
    • Small integral bias (like the prior designs) to cancel steady‑state error.
    Interface mirrors HingePOMDP so the rest of the code remains untouched.
    """
    def __init__(self, model, data, joint_name="hinge_joint",
                 dt=CTRL_DT,
                 tau_sat=POMDP_TAU_SAT,
                 sp_speed=POMDP_SP_SPEED_RAD_S,
                 J_est=POMDP_J_EST, D_est=POMDP_D_EST,
                 horizon=POMDP_H_CTRL, gamma=POMDP_GAMMA,
                 q_th=POMDP_W_TRACK, q_w=POMDP_W_VEL,
                 r_u=POMDP_W_U, r_du=POMDP_W_DU,
                 w_err_soft=POMDP_W_ERR_SOFT,
                 ki_bias=0.6, bias_limit_frac=0.35):
        self.model = model; self.data = data
        ...
        self.q_th  = float(q_th)
        self.q_w   = float(q_w)
        self.r_u   = float(r_u)
        self.r_du  = float(r_du)
        self.w_err_soft = float(w_err_soft)

        # Candidates (reuse ACTIONS grid for minimal diffs)
        self.u_cand = ACTIONS
        ...
        # Integral bias (steady-state cancellation)
        self.bias_u = 0.0
        self.Ki_bias = float(ki_bias)
        self.bias_limit = float(bias_limit_frac) * self.tau_sat

        # Keep fields for telemetry compatibility
        self.belief = [0.0]*N_REG
        self.params = POMDP_FRIC
    ...
    def enable_and_capture(self):
        ...
        self._a_hat = dq * 0.0
        self.enabled = True
        print(f"[hinge-MPC] Enabled at q={q:.4f} rad; τ_sat={self.tau_sat:.1f} N·m; "
              f"J={self.J_est:.4f}, D={self.D_est:.3f}, H={self.H}")
    ...
    def set_target_relative(self, dq: float):
        ...
        print(f"[hinge-MPC] Target REL → goal={self.sp_goal:.4f} rad")

    def set_target_absolute(self, q_abs: float):
        lo, hi = (-np.inf, np.inf)
        if self.model.jnt_limited[self.jid] == 1: lo, hi = self.model.jnt_range[self.jid]
        self.sp_goal = clamp(float(q_abs), lo, hi)
        print(f"[hinge-MPC] Target ABS → goal={self.sp_goal:.4f} rad")

    def _rollout_cost_const_u(self, th0: float, w0: float, u: float, sp_hold: float, u_prev: float) -> float:
        """Evaluate discounted cost over horizon with constant u."""
        th = th0; w = w0
        csum = self.r_du * (u - u_prev)**2  # Δu penalty once
        for h in range(self.H):
            e = (th - sp_hold)
            w_disc = (self.gamma**h)
            csum += w_disc * (self.q_th*(e*e) + self.q_w*(w*w) + self.r_u*(u*u) + self.w_err_soft*abs(e))

            # linear dynamics: θ̇ = w,  ẇ = (u - D·w)/J
            w = w + self.dt * ((u - self.D_est*w)/self.J_est)
            th = th + self.dt * w
        return float(csum)
    ...
    def step(self):
        ...
        if abs(err_goal) <= max_step: self.sp = self.sp_goal
        else:                         self.sp += math.copysign(max_step, err_goal)

        # Small integral bias to remove steady-state error
        e_now = (self.sp - q)
        self.bias_u += self.Ki_bias * e_now * dt
        self.bias_u = clamp(self.bias_u, -self.bias_limit, self.bias_limit)

        # Choose u via constant-input MPC over short horizon
        best_u = 0.0; best_c = float("inf")
        for u in self.u_cand:
            c = self._rollout_cost_const_u(q, dq, float(u), self.sp, self.last_tau_cmd)
            if c < best_c:
                best_c = c; best_u = float(u)

        # Apply bias, optional smoothing (alpha=0), and saturation
        u_raw = best_u + self.bias_u
        u_cmd = (1.0 - POMDP_U_SMOOTH_ALPHA) * u_raw + POMDP_U_SMOOTH_ALPHA * self.last_tau_cmd
        u_cmd = clamp(u_cmd, -self.tau_sat, self.tau_sat)

        # Write to MuJoCo
        self.data.qfrc_applied[self.vadr] += u_cmd
        self.last_tau_cmd = u_cmd

        # Acceleration estimate for telemetry
        a_raw = (dq - getattr(self, "_prev_v", dq)) / dt
        self._prev_v = dq
        self._a_hat = 0.2*a_raw + 0.8*getattr(self, "_a_hat", 0.0)

        # Risks (telemetry)
        self.last_risk_act = float(1.0 if abs(u_cmd) > TAU_MAX_FOR_RISK else 0.0)
        self.last_risk_err = float(1.0 if abs(e_now) > ERROR_RISK_THRESH_RAD else 0.0)


# =============================================================================
# ------------------------------ POMDP LOGIC (unused by MPC; kept) ------------
# =============================================================================
...
class HingePOMDP:
    """Placeholder kept for interface parity; unused when MPC is active."""
...
class HingeLoad:
    """Sinusoidal resistance torque applied at the hinge (time‑based)."""
    def __init__(self, model, data, joint_name="hinge_joint",
...
class GraspSM:
    ...
    3) Settle   → release hinge freeze; enable MPC; start sinusoidal load
    4) Drive
    """
    def __init__(self, model, data, ur: UR10, hand: BarrettHand,
                 ...
                 hinge_ctrl: HingeMPC, hinge_load: HingeLoad,
    ...
    def step(self, sim_time: float, dt: float):
        ...
        if self.phase == Phase.PREGRASP:
            ...
            if self.t_latched is not None and (sim_time - self.t_latched) >= POST_LATCH_SETTLE_S:
                ...
                print("[phase] Hinge drive active (POMDP).")  # kept text to minimize diffs
    ...
    def _maybe_post_plot(self, t: float):
        ...
        a_hat   = float(getattr(self.hinge, "_a_hat", 0.0)) if hinge_en else 0.0
        tau_dyn = float(getattr(self.hinge, "J_est", 0.0)*a_hat + getattr(self.hinge, "D_est", 0.0)*dq) if hinge_en else 0.0
        tau_cmd = float(getattr(self.hinge, "last_tau_cmd", 0.0)) if hinge_en else 0.0
        tau_load = float(self.hload.last_tau) if self.hload.enabled else 0.0
        bias_u   = float(getattr(self.hinge, "bias_u", 0.0)) if hinge_en else 0.0

        # expected friction torque (belief-weighted) — zeros under MPC
        tau_f_exp = 0.0
        if hinge_en and hasattr(self.hinge, "params"):
            for j in range(N_REG):
                tau_f_exp += (self.hinge.belief[j] if j < len(self.hinge.belief) else 0.0) * \
                             friction_torque(angle, dq, j, self.hinge.params)
        ...
        # risks
        risk_act = float(getattr(self.hinge, "last_risk_act", 0.0)) if hinge_en else 0.0
        risk_err = float(getattr(self.hinge, "last_risk_err", 0.0)) if hinge_en else 0.0
        risk     = max(risk_act, risk_err)

        # belief (flatten)
        b = list(getattr(self.hinge, "belief", [0.0]*N_REG)) if hinge_en else [0.0]*N_REG
        ...
        pkt = dict(
            ...
            J_est=float(getattr(self.hinge, "J_est", 0.0)), D_est=float(getattr(self.hinge, "D_est", 0.0)),
            b=b,
        )
        try:
...
def simulation_thread(model, data, total_time_s, realtime, plotter_ui: LivePlotterUI):
    ...
    # *** THE ONLY FUNCTIONAL CONTROLLER DIFFERENCE: use MPC instead of POMDP ***
    hinge_ctrl = HingeMPC(model, data, joint_name=FREEZE_PANEL_JOINT,
    ...
    # Use the TIME‑BASED sinusoidal load from your version
